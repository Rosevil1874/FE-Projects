...
<template>
    <div>
        <Row class="margin-top-10">
            <Card>
                ...
                <div class="edittable-table-height-con">
                    <maintain-table v-model="editData" :columns-list="editColumns" @on-change="handleChange" @on-delete="handleDel"></maintain-table>
                </div>
        ...
        <Modal v-model="addItemModal" :width="500">
            <h3 slot="header" style="color:#2D8CF0">新增维护记录</h3>
            <Form ref="addItemForm" :model="addItemForm" :label-width="100" label-position="right">
                ...
                <FormItem label="损坏时间">
                    <Input v-model="addItemForm.damageDate" placeholder="" ></Input>
                </FormItem>
                <FormItem label="监控类型">
...
import tableData from './components/maintain_data.js';
export default {
    name: 'editable-table',     // 不能更改！！！？？？
    components: {
        maintainTable
    },
    data () {
    ...
    methods: {
        ...
        getData () {
            let data = []
            this.$ajax.get('http://localhost/sewer-system/sewerPHP/monitor-management/getMaintain.php')
                .then(res => {
                    ...
                    for (var i = 0; i < res.data.length; i++) {
                        let obj = {
                            ...
                            damageDate: res.data[i].damageDate,
                            maintainDate: res.data[i].maintainDate,
        ...
        handleSearch () {
            ...
            this.editData = this.search(this.editData, {id: this.searchConName});
        },
        handleDel (val, index) {
        ...
        addItem (name) {
            let data = {
                ...
                'damageDate':this.addItemForm.damageDate,
                'type':this.addItemForm.type
            }
            this.$ajax.post('http://localhost/sewer-system/sewerPHP/monitor-management/add-maintain.php', 
                    {
                        data:data
                    },
                    {
