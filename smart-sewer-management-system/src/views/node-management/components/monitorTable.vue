...
const detailButton = (vm, h, currentRow, index) => {
    return h('Button', {
        ...
        on: {
            'click': () => {
                util.openNewPage(vm, 'detail_index');
                vm.$router.push({
                     name: 'detail_index',
                     params: {
                         id: vm.thisTableData[index].deviceId
                     }
                 });
            }
        }
        ...
        on: {
            'click': () => {
                if (!currentRow.editting) {
                    ...
                    // 更新数据库中修改项
                    Vue.prototype.$ajax.post('http://localhost/sewer-system/sewerPHP/monitor-management/edit-monitor.php',
                        {
                            data: edittingRow
                        },
                        {
                        ...
                        if (res.data !== 1) {
                            Vue.prototype.$Message.error('同步更新数据库失败')
                        }
                    })
                }
            }
    ...
    return h('Poptip', {
        ...
        on: {
            'on-ok': () => {
                let edittingRow = vm.edittingStore[index];
                // 删除数据库中删除项
                Vue.prototype.$ajax.post('http://localhost/sewer-system/sewerPHP/monitor-management/delete-monitor.php',
                    {
                        data: {'deviceId': vm.thisTableData[index].deviceId}
                    },
                    {
                        // 允许跨域
                        headers: {
