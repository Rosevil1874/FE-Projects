...
export default {
    ...
    mounted () {
        let phpURL = ''
        if (this.type === '水位') {
            phpURL = 'http://localhost/sewer-system/sewerPHP/statistic/get-water-inrange.php'
        } else {
            phpURL = 'http://localhost/sewer-system/sewerPHP/statistic/get-sewer-inrange.php'
        }
        let data = {
            start: this.startDate,
            end: this.endDate
        }
        this.$ajax.post( phpURL,
            {
                data:data
            },
            {
